  ...
  private void startRecordThread() {
    new Thread(() -> {
      ...
      while (startRecord) {
        ...
        short[] buffer1 = new short[buffer.length / 2];
        short[] bufferLeft = new short[buffer.length / 2];
        short[] bufferRight = new short[buffer.length / 2];
        int read = record.read(buffer, 0, buffer.length);


        for(int i = 0; i < buffer.length / 2; i++) {
          buffer1[i] = (short) (buffer[2 * i + 1] - buffer[2 * i]);  // 右声道-左声道
          bufferLeft[i] = buffer[2 * i];
          bufferRight[i] = buffer[2 * i + 1];
        }
//        if(Math.abs(calculateDb(buffer) - calculateDb(buffer1)) > 0.1 * calculateDb(buffer)) {
        if(calculateDb(buffer1) < 0.4 || (calculateDb(buffer1) > 0.4 && Math.abs(calculateDb(bufferLeft) - calculateDb(bufferRight)) < 0.1)) {
          Log.e(LOG_TAG, calculateDb(buffer1) + "!!!!!!!!!!!!!" + calculateDb(bufferLeft) + "!!!!!" + calculateDb(bufferRight));
          for(int i = 0; i < buffer.length / 2; i++) {
            buffer1[i] = 0;
          }
        }
